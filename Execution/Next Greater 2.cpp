...
map<int, int> mymap; 
// find the next greater frequency element for each element and for placing it in the resultant array
void NGF(int arr[], int res[], int n) { 
    // Initially store the frequencies of all elements  in a hashmap 
    for(int i = 0; i < n; i++) { 
        mymap[arr[i]] += 1; 
    } 
    ...
    // place -1 as next greater freq for the last element as it does not have next greater. 
    res[n-1] = -1; 
