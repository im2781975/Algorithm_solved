...
// Function to perform binary search on array ar2[] for the closest element to x
...
// ar1[0..m-1] and ar2[0..n-1] are two given sorted arrays and x is given number.
...
int main()
{
    ...
    int x = 38;
    // Perform binary search on ar2[] for the element closest to x-ar1[i]
    for (int i = 0; i < m; i++)
    {
        int index = binarySearch(ar2, 0, n-1, x-ar1[i]);
        // Check if the element closest to x-ar1[i] is better than the current best
