// find k'th smallest element in a stream
#include <bits/stdc++.h>
using namespace std;
...
// method to heapify a subtree with root at given index This method assumes that the subtrees are already heapified
    ...
    for (auto& x : A) {
        ...
        if (count < k - 1) {
            ...
            if (count == k - 1) {
                ...
                // If next element is greater than k'th largest, then replace the root
                if (x > mh.getMin())
