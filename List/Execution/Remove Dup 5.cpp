...
Node* removeDuplicates(Node* head)
{
    ...
    while (temp != NULL) {
        ...
        if (temp->data != prev->data) {
            // if the value of prev is not equal to the value of temp that means there are no more
            // occurrences of the prev data-> So we can set the next of prev to the temp node->*/
            prev->next = temp;
            prev = temp;
        }
