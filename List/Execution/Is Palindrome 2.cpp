...
bool isPalindrome(struct Node* head)
{
    ...
    if (head != NULL && head->next != NULL) {
        ...
        // fast_ptr would become NULL when there are even elements in list. And not NULL
        // for odd elements. We need to skip the
        // middle node for odd case and store it somewhere so that we can restore the original list
        if (fast_ptr != NULL) {
            midnode = slow_ptr;
            slow_ptr = slow_ptr->next;
        }
