...
// function to append a character x at the end of DLL. Note that the function may change head and tail
...
// A utility function to remove a node 'temp' from DLL. Note that the function may change the head and tail pointers,
...
void findFirstNonRepeating()
{
    ...
    for (int i = 0; stream[i]; i++) {
        ...
        // occurred or occurred only once. repeated[x] is true if x is repeated twice or more.s
        if (!repeated[x]) {
